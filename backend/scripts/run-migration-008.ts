/**
 * Applies db/migrations/008_gamification_badges.sql (run from backend/: npm run migrate:008).
 */
import fs from 'fs';
...
async function main() {
  const sqlPath = path.join(process.cwd(), 'db', 'migrations', '008_gamification_badges.sql');
