...
export async function register(
  ...
  const rows = await query<SafeUser[]>(
    `SELECT id, email, display_name, role, level, assessment_completed,
            ...
            is_active, is_premium, payment_reference, created_at, updated_at
     FROM users WHERE id = ?`,
    [id],
  );
...
export async function setPrimaryTrack(
  ...
  if (user.assessment_completed) {
    throw {
      ...
      message: "Placement is already complete.",
    };
  }
  await query("UPDATE users SET primary_track = ? WHERE id = ?", [
    track,
    userId,
  ]);
  return getMe(userId);
...
export async function resetPasswordWithToken(
  ...
  if (Number.isNaN(expiresAt.getTime()) || expiresAt.getTime() < Date.now()) {
    await query("DELETE FROM password_reset_tokens WHERE token_hash = ?", [
      tokenHash,
    ]).catch(() => {});
    throw {
      code: "TOKEN_EXPIRED",
