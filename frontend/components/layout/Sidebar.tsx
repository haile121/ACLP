    ...
    return (
      <nav className="flex flex-col gap-1 p-3 mt-4 flex-1">
        ...
        >
          ...
          {isCollapsed && (
            <div
              className="mx-auto mb-1 h-px w-8 rounded-full bg-gray-200 dark:bg-gray-700"
              aria-hidden
            />
          )}
        </div>
        ...
        {studentToolsItems.map(({ label, href, icon: Icon }) => (
          <div key={`${label}-${href}`}>{renderNavLink(label, href, Icon)}</div>
        ))}
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="hidden lg:flex p-4 border-t border-gray-100 dark:border-white/5 pb-6"
          suppressHydrationWarning
        >
          <button
            type="button"
