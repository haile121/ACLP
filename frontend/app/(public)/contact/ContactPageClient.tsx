...
const SOCIAL_ICONS: Record<
  (typeof CONTACT_SOCIAL_LINKS)[number]["id"],
  LucideIcon
> = {
  github: Github,
  x: Twitter,
...
export default function ContactPageClient() {
  ...
  const openMailto = () => {
    const subject = encodeURIComponent(
      `ACLP — message from ${name || (am ? "ጎብኚ" : "visitor")}`,
    );
    const body = encodeURIComponent(
      `${message}\n\n---\n${am ? "ስም" : "Name"}: ${name || "(not provided)"}\n${am ? "የመልስ ኢሜይል" : "Reply-to"}: ${email || "(not provided)"}`,
    );
        ...
        <div className="relative max-w-screen-xl mx-auto grid lg:grid-cols-12 gap-14 lg:gap-20 items-start">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white leading-[1.1]">
              {am ? "እያንዳንዱን መልዕክት እናነባለን" : "We read every message"}
            </h1>
            <p className="mt-6 text-lg text-gray-500 dark:text-gray-400 leading-relaxed">
              {am
                ? "ስህተቶችን ለመግለፅ፣ ለአጋርነት ሃሳቦች፣ ወይም አስተያየቶች—ይላኩልን። ለአካውንት ችግሮች ሲመዘገቡ የተጠቀሙበትን ኢሜይል ያካቱ።"
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder={am ? "የእርስዎ ስም" : "Your name"}
                  />
                </div>
                <div>
                  <label
                ...
                >
                  {am ? "በኢሜይል መተግበሪያ ይክፈቱ" : "Open in email app"}
                  <ArrowRight className="w-4 h-4" />
                </button>
                <p className="text-xs text-gray-400 dark:text-gray-500">
                  {am
                    ? "መልዕክቱን በማዘጋጀት ነባሪ የኢሜይል መተግበሪያዎን ይከፍታል። ምንም አይነት መረጃ ከሰርቨራችን አይላክም።"
